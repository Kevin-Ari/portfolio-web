...
card.innerHTML=`<h3>${p.title}</h3><p>${p.desc}</p><div class="tags">${p.tags.map(t=>`<span class="tag" onclick=\"filterTag('${t}')\">${t}</span>`).join('')}</div><div class="actions"><a class="live" href="${p.live}" target="_blank">Ver</a><a class="code" href="${p.code}" target="_blank">Código</a></div>`;
grid.appendChild(card);
}
}
function filterTag(tag){q.value='';render(projects.filter(p=>p.tags.includes(tag)));}
function resetFilters(){q.value='';render(projects);}
q.addEventListener('input',()=>{const term=q.value.trim().toLowerCase();if(!term)return render(projects);render(projects.filter(p=>(p.title+p.desc+p.tags.join(' ')).toLowerCase().includes(term)));});
render(projects);